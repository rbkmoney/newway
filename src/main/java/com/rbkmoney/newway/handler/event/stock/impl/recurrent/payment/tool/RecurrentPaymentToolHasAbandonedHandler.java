...
public class RecurrentPaymentToolHasAbandonedHandler implements RecurrentPaymentToolHandler {
    ...
    private final Filter filter =
            new PathConditionFilter(new PathConditionRule("rec_payment_tool_abandoned", new IsNullCondition().not()));
    ...
    public void handle(RecurrentPaymentToolChange change, MachineEvent event, Integer changeId) {
        ...
        recurrentPaymentToolNew.setStatus(RecurrentPaymentToolStatus.abandoned);
        recurrentPaymentToolDao.save(recurrentPaymentToolNew).ifPresentOrElse(
                ...
                () -> log.info("End recurrent payment tool abandoned bound duplicated, sequenceId={}, changeId={}",
                        sequenceId, changeId));
    }
