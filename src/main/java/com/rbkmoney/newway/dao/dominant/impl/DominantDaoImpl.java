    ...
    public Long getLastVersionId() throws DaoException {
        Query query = getDslContext().select(DSL.max(DSL.field("version_id"))).from(
                getDslContext().select(CALENDAR.VERSION_ID.max().as("version_id")).from(CALENDAR)
                        ...
                        .unionAll(getDslContext().select(PAYMENT_INSTITUTION.VERSION_ID.max().as("version_id"))
                                .from(PAYMENT_INSTITUTION))
                        .unionAll(getDslContext().select(PAYMENT_METHOD.VERSION_ID.max().as("version_id"))
                                .from(PAYMENT_METHOD))
                        .unionAll(getDslContext().select(PAYOUT_METHOD.VERSION_ID.max().as("version_id"))
                                .from(PAYOUT_METHOD))
                        ...
                        .unionAll(getDslContext().select(TERM_SET_HIERARCHY.VERSION_ID.max().as("version_id"))
                                .from(TERM_SET_HIERARCHY))
                        .unionAll(getDslContext().select(WITHDRAWAL_PROVIDER.VERSION_ID.max().as("version_id"))
                                .from(WITHDRAWAL_PROVIDER))
                        .unionAll(getDslContext().select(PAYMENT_ROUTING_RULE.VERSION_ID.max().as("version_id"))
                                .from(PAYMENT_ROUTING_RULE))
        );
