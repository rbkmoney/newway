...
public class DepositRevertTransferCreatedHandler extends AbstractDepositHandler {
    ...
    public void handle(TimestampedChange timestampedChange, MachineEvent event) {
        ...
        List<FinalCashFlowPosting> postings = change.getRevert().getPayload().getTransfer().getPayload().getCreated().getTransfer().getCashflow().getPostings();
        ...
        depositRevertDao.save(depositRevert).ifPresentOrElse(
                id -> {
                    ...
                    List<FistfulCashFlow> fistfulCashFlows = FistfulCashFlowUtil.convertFistfulCashFlows(postings, id, FistfulCashFlowChangeType.deposit_revert);
                    fistfulCashFlowDao.save(fistfulCashFlows);
                },
                () -> log.info("Deposit revert transfer have been saved, sequenceId={}, depositId={}", sequenceId, depositId)
        );
    }
