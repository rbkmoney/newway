...
public class DepositAdjustmentTransferCreatedHandler extends AbstractDepositHandler {
    ...
    public void handle(TimestampedChange timestampedChange, MachineEvent event) {
        ...
        List<FinalCashFlowPosting> postings = change.getAdjustment().getPayload().getTransfer().getPayload().getCreated().getTransfer().getCashflow().getPostings();
        long sequenceId = event.getEventId();
        String depositId = event.getSourceId();
        String adjustmentId = change.getAdjustment().getId();
        log.info("Start deposit adjustment transfer created handling, sequenceId={}, depositId={}", sequenceId, depositId);
        ...
        depositAdjustmentDao.save(depositAdjustment).ifPresentOrElse(
                id -> {
                    ...
                    List<FistfulCashFlow> fistfulCashFlows = FistfulCashFlowUtil.convertFistfulCashFlows(postings, id, FistfulCashFlowChangeType.deposit_adjustment);
                    fistfulCashFlowDao.save(fistfulCashFlows);
                },
                () -> log.info("Deposit adjustment transfer have been saved, sequenceId={}, depositId={}", sequenceId, depositId)
        );
    }
}
